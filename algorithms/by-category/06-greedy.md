...
### 1) Greedy корректен только с доказательством

Убедительная эвристика - не доказательство.

Основные техники доказательства:

1. Exchange argument.
2. Cut property.
3. Staying ahead argument.

Если не можешь сформулировать одну из техник, решение уязвимо.

### 2) Exchange argument (интуиция)

Показываем, что любой оптимум можно преобразовать так,
чтобы он начинался с greedy-выбора, не ухудшая качество.

Тогда greedy-шаг безопасен.

### 3) Типы задач, где greedy часто работает

1. Interval scheduling/covering.
2. Выбор по крайнему критерию (earliest finish, minimum edge across cut).
3. Задачи, где локальное решение "освобождает" максимум будущих возможностей.

### 4) Где greedy чаще всего ломается

1. 0/1 выборы с долгосрочной зависимостью (0/1 knapsack).
2. Когда локально лучший ход блокирует большую выгоду позже.
3. Когда решение требует глобального контекста, а не локального ранжирования.

### 5) Greedy vs DP: практический критерий

1. Если есть короткое доказательство безопасности локального шага -> greedy.
2. Если есть контрпример к greedy -> переход к DP/graph shortest path.

### 6) Опасный анти-паттерн

"Я попробовал на 3 примерах, работает".

Для greedy это особенно опасно, потому что контрпример часто маленький, но неочевидный.

### 7) Interview follow-up

1. Докажи корректность выбранной greedy-стратегии.
2. Приведи контрпример для похожей, но неверной greedy-идеи.
3. Сравни с DP-решением по времени/памяти/сложности реализации.

### 8) Контрольные вопросы

1. Почему fractional knapsack greedy, а 0/1 knapsack нет?
2. Почему сортировка по finish time максимизирует число интервалов?
3. В чём "безопасность" шага в Prim/Kruskal?
4. Как быстро проверить идею greedy на контрпример?
5. Когда heap делает greedy более естественным?
