...
### 1) Поиск как задача о predicate

Обобщённая постановка Binary Search:

- есть функция `P(x)`, монотонная по `x`;
- нужно найти границу перехода `false -> true`.

Это превращает множество задач в одну форму:
...
### 2) Инвариант - сердце Binary Search

Хороший binary search начинается с формального инварианта.

Пример для lower bound:

- все индексы `< left` точно не ответ;
- все индексы `>= right` потенциально включают ответ (или правую границу кандидатов).

Пока инвариант не выписан, off-by-one почти неизбежен.

### 3) Три канонических формы
...
- в интервью проще стандартизовать мышление вокруг lower/upper bound,
- exact match рассматривать как частный случай.

### 4) Почему `O(log n)` формально

После каждого шага остаётся максимум половина диапазона.

Через `k` шагов размер диапазона:
...
### 5) Search on Answer: строгий чек-лист

Можно применять, если:

1. Ответ лежит в числовом диапазоне `[L, R]`.
2. Есть проверка `feasible(x)`.
3. `feasible` монотонна.

Если монотонности нет, binary search on answer некорректен.

### 6) Частые ловушки senior-level

1. Неверная доменная граница `[L, R]` -> потеря ответа.
2. `mid` округляется не в ту сторону для выбранного инварианта.
3. Для float-задач нет критерия остановки (`eps`, итерации).
4. Массив отсортирован "логически", но с custom comparator есть нарушения.

### 7) Когда binary search не лучший выбор

1. Данные динамически меняются без поддержки порядка.
2. Поиск один раз на маленьком массиве (линейный проще и практичнее).
3. Predicate дорогой и не монотонный.

### 8) Контрольные вопросы

1. Почему без монотонности нельзя выбрасывать половину?
2. Чем lower bound концептуально отличается от exact match?
3. Какие инварианты ты выбираешь для `left < right` и для `left <= right`?
4. Когда exponential search предпочтительнее обычного binary search?
5. Как корректно бинпоискать по double-ответу?
