...
### 1) Две оси строковых задач

1. Matching (найти вхождения).
2. Structure (палиндромы, редакционное расстояние, сегментация).

Алгоритм выбирается по тому, что нужно оптимизировать:

- много сравнений подстрок,
- много DP-переходов,
- много prefix/suffix вычислений.

### 2) KMP: ключевая интуиция

При mismatch мы не "теряем" информацию о совпавшем префиксе.

LPS/Prefix-function говорит, на сколько безопасно сдвинуть pattern,
не пересматривая символы, про которые уже известно, что они совпали.

Именно это даёт линейность `O(n+m)`.

### 3) Z-function vs Prefix-function

Обе структуры кодируют повторяющуюся prefix-информацию,
но удобны в разных преобразованиях задач.

Полезно понимать взаимосвязь, чтобы гибко выбирать инструмент.

### 4) Rabin-Karp и коллизии

Hash ускоряет сравнение, но не гарантирует равенство строк.

Практики повышения надёжности:

1. double hashing,
2. большой модуль,
3. финальная проверка при hash-match.

### 5) Палиндромы: substring vs subsequence

Это разные задачи:

1. Substring - непрерывный фрагмент.
2. Subsequence - можно пропускать символы.

Отсюда разные методы:

- Expand/Manacher для substring.
- DP/LCS для subsequence.

### 6) Trie vs Hashing для словарных задач

Trie:

- лучше для prefix-операций,
- дороже по памяти.

Hash:

- проще и компактнее,
- не даёт prefix traversal "из коробки".

### 7) Interview follow-up

1. Как искать множество паттернов сразу? (Aho-Corasick как идея)
2. Как минимизировать память в DP для edit distance?
3. Что делать с Unicode/normalization проблемами?

### 8) Контрольные вопросы

1. Почему KMP линейный, несмотря на откаты j?
2. Почему hash-match недостаточен без защиты от коллизий?
3. Чем substring-задачи отличаются от subsequence-задач алгоритмически?
4. Когда trie оправдан несмотря на memory overhead?
5. Почему edit distance - это классический 2D DP?
