...
### 1) Когда два указателя дают линейность

Two pointers эффективен, когда движение хотя бы одного указателя монотонно.

Ключ:

- каждый указатель сдвигается ограниченное число раз (обычно <= n),
- значит общая работа `O(n)`, а не `O(n^2)`.

### 2) Opposite pointers: аргумент корректности

Для отсортированного массива и суммы:

- если `a[l] + a[r]` слишком мала, увеличение `l` - единственный шанс увеличить сумму;
- если слишком велика, уменьшение `r` - единственный шанс уменьшить сумму.

Это и есть причина, почему не нужно пробовать все пары.
...
- `fast` исследует,
- `slow` фиксирует границу валидной/сжатой части.

Инвариант формулируется как:

- префикс `[0..slow)` уже приведён к требуемому виду.

### 4) Floyd cycle detection: почему работает

Если есть цикл:

- fast догонит slow внутри цикла (задача о движении по кольцу с разной скоростью).

Для поиска старта цикла:

- после встречи один указатель ставим в head;
- оба идут со скоростью 1;
- встретятся в начале цикла.

Это следует из равенства пройденных расстояний по модулю длины цикла.

### 5) Дубликаты и канонический порядок

В задачах `3Sum/4Sum` обязательна стратегия skip duplicates,
иначе получишь повторные решения и неверный output format.

### 6) Когда two pointers не работает

1. Нет монотонности при движении указателей.
2. Данные не отсортированы, а сортировать нельзя (важен исходный порядок и индексы).
3. Условие зависит от "разбросанных" позиций, а не локальной пары/окна.

### 7) Interview follow-up

1. Как адаптировать под streaming input?
2. Можно ли решить без сортировки и что потеряем?
3. Как обобщить с 2Sum на kSum?

### 8) Контрольные вопросы

1. Почему движение меньшей высоты корректно в Container With Most Water?
2. Почему Remove Duplicates in-place линейный?
3. В чём разница между two pointers и sliding window?
4. Как доказывается корректность cycle start алгоритма?
5. Когда лучше HashMap вместо two pointers?
