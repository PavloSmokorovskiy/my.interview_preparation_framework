...
### 1) Two's complement как фундамент

Большинство битовых трюков опирается на представление signed int в two's complement.
...
Понимание этого снимает "магичность" многих формул.

### 2) Bitwise operators как алгебра над множествами

Битовую маску можно мыслить как множество флагов:

1. `|` - объединение,
2. `&` - пересечение,
3. `^` - симметрическая разность,
4. `~` - дополнение (в рамках размера слова).

### 3) Почему `x & (x-1)` работает

- `x-1` инвертирует младший установленный бит и все младшие нули/единицы справа;
- AND с `x` стирает именно этот младший установленный бит.
...
### 4) Bitmask state compression

Если `n` небольшое (`<= 20..22` обычно), подмножество удобно кодировать битмаской.

Это превращает экспоненциальный перебор в контролируемый DP по маскам.

### 5) Частые pitfalls

1. Сдвиги на signed типах (аритметический vs логический).
2. Overflow при `1 << k` если `k` близко к разрядности.
3. Неверный тип (`int` vs `long`) в задачах с 64-битными значениями.

### 6) Когда bit operations дают реальное преимущество

1. Компактное хранение множества состояний.
2. Очень быстрые low-level проверки флагов.
3. Оптимизация constant factors в tight loops.

### 7) Interview follow-up

1. Как итерировать все submasks mask?
2. Как за `O(1)` проверить subset relation для масок?
3. Когда выбирать bitset/bitmap вместо HashSet?

### 8) Контрольные вопросы

1. Почему `x & -x` выделяет младший бит?
2. Почему `n & (n-1) == 0` означает степень двойки?
3. Когда bitmask DP практичен, а когда нет?
4. Какие риски у signed shift?
5. Почему XOR удобен для "парами исчезающих" элементов?
