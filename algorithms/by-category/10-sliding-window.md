...
### 1) Sliding Window как инкрементальное поддержание состояния

Окно - это не просто `[l, r]`, а структура состояния:

1. счётчики,
2. текущая метрика (sum/max/min/violations),
3. условия валидности.

Сложность `O(n)` достигается благодаря инкрементальным обновлениям при add/remove.

### 2) Два главных семейства задач

1. Maximize length under constraint (обычно "at most K").
2. Minimize length satisfying requirement (обычно "at least / contains all").

Они похожи внешне, но различаются условием shrink.

### 3) Invariant design

Перед реализацией выпиши:

1. Что означает "valid window".
2. Когда нужно расширять окно.
3. Когда нужно сжимать.
4. В какой момент обновляется ответ.

Если эти пункты не определены, ошибки почти неизбежны.

### 4) Почему обычно O(n)

- правый указатель двигается максимум `n` раз;
- левый указатель двигается максимум `n` раз;
- операции обновления состояния `O(1)` или амортизированно `O(1)`.

### 5) Monotonic deque: строгая идея

Для max-window:

- deque хранит индексы в порядке убывания значений;
- фронт всегда максимум текущего окна;
...
Часто задачи "exactly K distinct" решают через:

- `atMost(K) - atMost(K-1)`.

Это важный interview-паттерн высокого уровня.

### 7) Когда sliding window не подходит

1. Нужны non-contiguous subsequences.
2. Условие не поддерживается инкрементально при remove-left.
3. Нет монотонной логики shrink/expand.

### 8) Контрольные вопросы

1. Почему minimum-window задачи обычно требуют while-shrink?
2. Когда нужна HashMap, а когда достаточно массива счётчиков?
3. Почему monotonic deque не теряет кандидатов на максимум?
4. Как решать exactly-K distinct через atMost?
5. Что ломается, если забыть удалить элемент при сдвиге left?
