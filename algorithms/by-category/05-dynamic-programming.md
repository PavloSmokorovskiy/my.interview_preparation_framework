...
### 1) Правильная формулировка state

State должен быть:

1. Минимально достаточным для перехода.
2. Однозначно определяемым.
3. Не дублировать эквивалентные состояния.

Плохой state ведёт к:

- лишним размерностям,
- взрыву памяти,
- запутанным переходам.

### 2) Корректность DP через индукцию

Стандартная схема доказательства:

1. Base case корректен.
2. Предположим, все меньшие состояния корректны.
3. Transition использует только корректные меньшие состояния.
4. Следовательно, текущее состояние корректно.

Так DP-решение становится доказуемым, а не "похоже работает".

### 3) Три главных вопроса перед кодом

1. Что означает `dp[i]` или `dp[i][j]` буквально?
2. От каких предыдущих состояний оно зависит?
3. В каком порядке заполнять таблицу, чтобы зависимости уже были посчитаны?

### 4) Выбор измерений

Часто одна ось - это "прогресс по input", другая - "ресурс/ограничение".

Примеры:

1. Knapsack: `i` (сколько предметов) + `w` (вес).
2. String DP: `i, j` (префиксы двух строк).
3. Grid: `r, c` (координаты).

### 5) Memory optimization без потери смысла

Можно сжимать только если:

- переход использует ограниченное "окно" прошлых состояний.

Перед сжатием проверяй:

1. Не перезаписываешь ли значение раньше времени.
2. Какой нужен порядок обхода (left->right или right->left).

### 6) Когда DP не нужен

1. Нет overlapping subproblems.
2. Greedy имеет строгое доказательство.
3. Простой graph traversal уже даёт оптимум.

DP - мощный, но не всегда лучший по простоте.

### 7) Interview follow-up

1. Можно ли уменьшить память с `O(nm)` до `O(min(n,m))`?
2. Как восстановить путь/решение, а не только значение optimum?
3. Как обработать очень большой `n` (state compression, math, monotonic queue optimization)?

### 8) Контрольные вопросы

1. Что сломается при неправильном base case?
2. Почему один и тот же transition может требовать разный порядок обхода?
3. Как понять, что состояние избыточно?
4. Почему "DP = рекурсия" - неверное упрощение?
5. Когда top-down лучше bottom-up, и наоборот?
