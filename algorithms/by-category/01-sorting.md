...
### 1) Нижняя граница для сравнительных сортировок

Для любой сортировки, которая использует только сравнения элементов, есть нижняя граница:

- worst-case `Omega(n log n)`.

Интуиция:

- сортировка должна различить `n!` перестановок;
- дерево сравнений должно иметь минимум `n!` листьев;
- высота дерева минимум `log2(n!) = Omega(n log n)`.

Следствие:

- нельзя сделать "магическую" сравнительную сортировку `O(n)` для общего случая.
- `O(n)` возможно только при дополнительных предположениях о данных (Counting/Radix/Bucket).

### 2) Стабильность, in-place и locality

Три свойства, которые часто важнее чистого Big-O:

1. Stable: равные ключи сохраняют относительный порядок.
2. In-place: мало дополнительной памяти.
3. Cache locality: насколько алгоритм дружит с CPU cache.

Практические эффекты:

- Merge: стабилен, но требует дополнительную память.
- Quick: обычно быстрый за счёт locality, но не стабилен.
- Heap: in-place и гарантированный, но locality хуже.

### 3) Инварианты корректности (короткие доказательства)

#### Insertion Sort

Инвариант после итерации `i`:

- подмассив `[0..i]` отсортирован и содержит те же элементы, что исходно.

Шаг поддержания:

- вставка `arr[i+1]` в правильную позицию сохраняет отсортированность.

#### Merge Sort

Инвариант merge-шага:

- перед каждым выбором минимального элемента из двух отсортированных половин,
- выход уже отсортирован и содержит минимальные из ещё не взятых.

#### Quick Sort

Инвариант partition:

- элементы слева от границы `< pivot`, справа `>= pivot` (или другая выбранная конвенция).

После завершения partition pivot стоит в финальной позиции.

### 4) Почему Quick Sort иногда `O(n^2)`

Если pivot систематически плохой (например, всегда минимум/максимум), разбивка становится:
...
1. random pivot,
2. median-of-three,
3. introsort fallback на heapsort.

### 5) Когда выбирать не сортировку, а selection

Если нужна только статистика порядка (`k`-й минимум/максимум), полная сортировка часто избыточна.

- Full sort: `O(n log n)`
- Quickselect average: `O(n)`
- Heap of size `k`: `O(n log k)`

### 6) Interview follow-up, которые нужно уметь закрывать

1. Что выбрать для почти отсортированных данных?
- Insertion/Timsort.

2. Что выбрать при жёстком лимите памяти?
- HeapSort или careful quicksort.

3. Что выбрать, если нужна стабильность?
- Merge/Timsort.

4. Что выбрать для потока данных?
- Обычно heap/online-структуры, не batch sorting.

5. Что выбрать для huge external data?
- External merge sort.

### 7) Контрольные вопросы (без кода)

1. Почему `O(n log n)` - барьер для сравнительных сортировок?
2. Почему Quick Sort обычно быстрее Heap Sort на практике?
3. Почему Merge Sort популярен для linked list?
4. Когда Counting Sort лучше Merge/Quick?
5. Почему стабильность важна для multi-key сортировки?
