...
### 1) Tree invariants как язык доказательства

Для каждой tree-структуры есть свой инвариант:

1. BST order invariant.
2. Heap order invariant.
3. Balanced-tree height invariant.
4. Trie prefix invariant.

Без явного инварианта трудно доказывать корректность операций.

### 2) Балансировка и гарантия логарифма

`O(log n)` для BST операций справедливо только при контроле высоты.

AVL/RB-tree вводят ограничения, которые не дают дереву деградировать в список.

### 3) Trie trade-off

Trie ускоряет prefix задачи до `O(m)`,
но платит памятью и overhead на узлы.

Для редких запросов или малого словаря hash-based решения могут быть лучше.

### 4) Segment/Fenwick как "деревья агрегации"

Обе структуры хранят агрегаты диапазонов,
но отличаются гибкостью и сложностью реализации.

Segment tree:

- универсальнее по операциям.

Fenwick:

- проще и компактнее для prefix-like агрегаций.

### 5) Контрольные вопросы

1. Почему локальная проверка BST через parent не работает?
2. Зачем balanced trees, если есть hash maps?
3. Когда trie оправдан по памяти/скорости?
4. Как выбрать между segment tree и fenwick?
5. Какой инвариант критичен для delete в BST?
